...
### 1. 🎮 게임 백로그 (Game Backlog) — 풀스택 + 배포 ⭐⭐
- **React + Express + SQLite** 로 만든 게임 관리 앱. **인터넷에 배포까지 완료.**
- **🌐 라이브 데모:** https://game-backlog-web.onrender.com
- 완전한 **CRUD**: 게임 추가(POST)·조회(GET)·상태변경/별점(PUT)·삭제(DELETE)
- **SQLite DB** 영구 저장, 상태별 필터·통계, 별점(⭐) 평가, 반응형 UI
- **Render** 로 프론트/백엔드 각각 배포, `process.env.PORT`·환경별 설정 적용
- 폴더: `react-app/` (프론트), `server/` (백엔드 API + DB)

### 2. 풀스택 CRUD 앱 (React + Express)
